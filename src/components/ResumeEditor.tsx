...
export default function ResumeEditor({ data, editorRef }: ResumeEditorProps) {
  const localRef = useRef<HTMLDivElement>(null);
  const contentRef = editorRef || localRef;
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      const { generateDocxBlob, getResumeJsonFromDOM } = await import("@/lib/docxHelper");
      const editedJson = getResumeJsonFromDOM(contentRef.current);
      const blob = await generateDocxBlob({ ...data, resumeJson: editedJson });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Resume_${editedJson.name?.replace(/\s+/g, "_") || "Tailored"}.docx`;
